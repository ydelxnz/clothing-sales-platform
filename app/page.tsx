                ...
                <div className="bg-card rounded-xl overflow-hidden border">
                  ...
                  <div className="p-4">
                    <div className="flex mb-4 border-b pb-2">
                      <button
                        className={`px-4 py-2 text-sm font-medium ${activeTab === 0 ? "text-primary border-b-2 border-primary -mb-[9px]" : "text-muted-foreground"}`}
                        onClick={() => setActiveTab(0)}
                      >
                        销售预测
                      </button>
                      <button
                        className={`px-4 py-2 text-sm font-medium ${activeTab === 1 ? "text-primary border-b-2 border-primary -mb-[9px]" : "text-muted-foreground"}`}
                        onClick={() => setActiveTab(1)}
                      >
                        定价策略
                      </button>
                      <button
                        className={`px-4 py-2 text-sm font-medium ${activeTab === 2 ? "text-primary border-b-2 border-primary -mb-[9px]" : "text-muted-foreground"}`}
                    ...
                    <div className="h-[300px] relative overflow-hidden">
                      <div
                        className={`absolute inset-0 transition-opacity duration-500 ${activeTab === 0 ? "opacity-100" : "opacity-0"}`}
                      >
                        <img
                          src="/销售预测.png?height=300&width=500&text=销售预测图表"
                          alt="销售预测图表"
                          className="w-full h-full object-cover rounded-md"
                        />
                      </div>
                      <div
                        className={`absolute inset-0 transition-opacity duration-500 ${activeTab === 1 ? "opacity-100" : "opacity-0"}`}
                      >
                        <img
                          src="/定价策略.png?height=300&width=500&text=定价策略分析"
                          alt="定价策略分析"
                          className="w-full h-full object-cover rounded-md"
                        />
                      </div>
                      <div
                        className={`absolute inset-0 transition-opacity duration-500 ${activeTab === 2 ? "opacity-100" : "opacity-0"}`}
                      >
                        <img
                          src="/库存管理.png?height=300&width=500&text=库存管理优化"
                          alt="库存管理优化"
                          className="w-full h-full object-cover rounded-md"
                        />
                      </div>
